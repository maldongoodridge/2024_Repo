function [Network_failure_test,...         % BINARY: test for under frequency load shedding during simulation
          ...
          incremental_attack...  %each effective attack at each time
                    ...
                    attack_list,...   %this is the list of load attack (scaled by initial load and 100MVA base
...
numbat Uagc Lagc NN M D B mem Pb agc B0 Pbat connector_power con_relay ...
...
cyberattack maxload_ratio cyber_interval cinterval next_cyber_attack vulnerable_ratio...
interconnectors volt_delay volt_dip_threshold uvls_trip ls_times
...
j2 = @(gov,u,D,M,NN,X,Eref,Be,T,power,elec,...
    ...
    shutoff'.*aq'.*(-govdroop'.*gov(NN+1:2*NN)); %GOVERNOR
    ]; 
%______________________________________________________________________________________________________________________________

%------------------------------------------------------------------INPUT VARIABLES-----------------------------------------------------

B = be; M=m;Local_Gens = LG;Local_Load = LL;power = pw;NN = nn; loads = lds;
vulnerable_ratio = vr; maxload_ratio =mlr; cyber_interval = ci;
areas = area; Gen_M = gm; line_index = line_ind;
interconnectors = intercons; T = tt;
...
maxN = Local_Gens.*[9 9 9 9 9 9 10 10 15 15 zeros(1,NN-10)] + Local_Load*0;
...
gen_trip = ~Local_Gens;
line_trip = zeros(1,4); %there are 4 interconnectors to be tripped
Time_List = [];  
Rocof_List = [];
volt_list = [];
freq_list =[];
...
sequence = zeros(300,4*NN+length(line_trip)+1);
...
j = j+1;
regime = 1; 
[td,bat,~,~,failuretype] = ode45(@(td,bat) j2(bat,u,D,M,NN,X,Eref,B,T,power,elec,Pb,aq,...
...
if sum(con_relay)>0
    B = line_disconnection(con_relay,B,line_index);
end

if sum(rocof_loc+gen_shed_loc)>0        %tests for generation shedding event
    Gen_M(rocof_loc|gen_shed_loc) = 0.05 ;  %set the inertia constant at the lost generator to approx 0;
end
...
if sum(con_relay) + sum(rocof_loc+gen_shed_loc)+ sum(ufls_event)>0
    [M,D] = islanding(line_trip,Gen_M,fixed_D,loads,areas,pre_dist_loads,Local_Gens);
end

if any(failuretype==113) %this is when the next attack is scheduled to occur
...
nodalFailure = [rocof_tally zeros(1,4);OFGS_tally zeros(1,4);UFLS_tally zeros(1,4); uvls_trip zeros(1,4); zeros(1,NN) line_trip];
...
global NN gen_trip Local_Gens  power_change line_power_limit...
    ...
    cyber_attack interconnectors Local_Load volt_dip_threshold uvls_trip
...
line_freq = ma_freq(interconnectors);   %checks the frequency at the ends of each of the interconnector lines
line_freq = any(abs([line_freq(1,:); mean(line_freq(2:4,:)); line_freq(5,:);mean(line_freq(6:7,:))]')<1,1) ;  %returns 0 if frequency abs freq is less than 1Hz, but fails if it exceeds
...
% find(~position)
term     = [ones(1,NN),ones(1,NN),ones(1,NN) ones(1,4) ones(1,NN),1]';
direction = [];   
...
global Time_List Rocof_List u power regime rocof...
    elec Pb NN M D B minN maxN freq_list Local_Gens connector_power connect_relay_delay...
...
if regime == 0 
   ...
   Time_List(end) = t;
   Rocof_List(end,:) = abs(rocof(sol,NN,M,D,u,power,elec,B,Pb,minN,maxN,FD,Local_Gens,loads,cyberattack,vulnerable_ratio,maxload_ratio,Pbat))'/rad_to_hertz;
   freq_list(end,:) = sol(NN+1:2*NN)'/rad_to_hertz;
   volt_list(end,:) = sol(2*NN+1:3*NN);

   connector_power(end,:) =[ abs(sol(2*NN+15)*sol(2*NN+16)*B(15,16)*sin(sol(15)-sol(16))),...
                      sum([abs(sol(2*NN+10)*sol(2*NN+1)*B(10,1)*sin(sol(10)-sol(1))),...
                      abs(sol(2*NN+10)*sol(2*NN+21)*B(10,21)*sin(sol(10)-sol(21))),...
                      abs(sol(2*NN+10)*sol(2*NN+26)*B(10,26)*sin(sol(10)-sol(26)))]),...
                      abs(sol(2*NN+11)*sol(2*NN+26)*B(11,26)*sin(sol(11)-sol(26))),...
                      sum([abs(sol(2*NN+16)*sol(2*NN+23)*B(16,23)*sin(sol(16)-sol(23))),...
                      abs(sol(2*NN+16)*sol(2*NN+27)*B(16,27)*sin(sol(16)-sol(27)))])
                      ]; 
   ...
   Rocof_List = [Rocof_List; abs(rocof(sol,NN,M,D,u,power,elec,B,Pb,minN,maxN,FD,Local_Gens,loads,cyberattack,vulnerable_ratio,maxload_ratio,Pbat))'/rad_to_hertz];
...
%    if con_relay<1
    connector_power = [connector_power;
                      abs(sol(2*NN+15)*sol(2*NN+16)*B(15,16)*sin(sol(15)-sol(16))),...
                      sum([abs(sol(2*NN+10)*sol(2*NN+1)*B(10,1)*sin(sol(10)-sol(1))),...
                      abs(sol(2*NN+10)*sol(2*NN+21)*B(10,21)*sin(sol(10)-sol(21))),...
                      abs(sol(2*NN+10)*sol(2*NN+26)*B(10,26)*sin(sol(10)-sol(26)))]),...
                      abs(sol(2*NN+11)*sol(2*NN+26)*B(11,26)*sin(sol(11)-sol(26))),...
   ...
   if t>connect_relay_delay
       %THIS RESETS THE CONNECTOR POWER RELAY LIST
       connector_power(1,:) = []; 
   end
       power_change = (connector_power(end,:) - connector_power(1,:))*(t>connect_relay_delay);
%    end
...
rocof_loc = zeros(1,NN); conn_relay_trip = zeros(1,4); gen_shed_loc = zeros(1,NN);
...
conn_relay_trip(failuretype(failuretype>4*NN & failuretype<=4*NN+4)-4*NN) = 1;  %this calculates where line trips occurs
