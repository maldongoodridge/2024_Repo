function Adjusted_Skipping_Algorithm_IEEE39_particles_cyclical(...
    q,network_tune,start,NR,NN,number,Be,Local_Gens,Local_Load,M,...
    power,loads,areas,Gen_M,interconnectors,line_index,T, ...
    initial_conditions,initial_value,nom_freq_range,...
    dist_location,maximum_load_ratios,tuning_attack,...
    initial_attack,network_skips,particle_skips)      %vector of battery location in the network)
...
for i = 1:number
    ...
    while counter<=network_skips

        particles = current_particles;
        old_no_attacks = size(particles,1);

        network_distance = exprnd(network_tune,1);
        proposal = proposal + network_direction*network_distance;
        pre_corrected_proposal = proposal;
        ...
        trajectories = randn(no_of_attacks,19);
        particle_directions = trajectories./vecnorm(trajectories,2,2);
        subcounter=1;
        
        while subcounter<=particle_skips
        
            particle_jump =  exprnd(tuning_attack,no_of_attacks,1);%tuning_attack*rand(no_of_attacks,1);% 
            a = particles + particle_jump.*particle_directions;
            particles = a;
            particles = cyclical_box(particles,0,5,"particles");
    
            r = rand(1,28).*[1 zeros(1,27)];
            ...
            Incremental_Attack...         % each effective cyber attack during simulation
                            ...
                            0,...                   %vector of battery maximum power outputs/inputs
...
%     
    %         accept = Region*lognpdf(sum(prop_cumulative_attack(Local_Load)),1,6)/...
     %                (lognpdf(sum(current_cumulative_attack(Local_Load)),1,6));	
    ...
    if accept > rand()
        ...
        network_attacks{end+1} = [1 1 60].*proposal;
